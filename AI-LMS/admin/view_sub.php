...
if ($action === 'fetch') {
    $stmt = $conn->prepare("SELECT * FROM subject_tbl WHERE sub_id = ?");
...
if ($action === 'update') {
    $stmt = $conn->prepare("UPDATE subject_tbl SET class = ?, subject = ? WHERE sub_id = ?");
    $stmt->bind_param("ssi", $_POST['class'], $_POST['subject'], $_POST['sub_id']);
  ...
  <body>
          ...
          <div class="page-inner">
            ...
            <?php
              ...
              if (isset($_GET['delete_id'])) {
                  ...
                  // Delete query
                  $query = "DELETE FROM `subject_tbl` WHERE `sub_id`='$sub_id'";
                  $result = mysqli_query($conn, $query);

                  if ($result) {
                      echo "<script>alert('Subject Details deleted successfully!'); window.location.href='view_sub.php';</script>";
                  } else {
                      echo "<script>alert('Error deleting subject details: " . mysqli_error($conn) . "');</script>";
                  }
              }
            ?>
                ...
                <div class="card">
                  <div class="card-header">
                    <div class="d-flex align-items-center">
                      ...
                      <a href="add_sub.php" class="btn btn-primary btn-round ms-auto">
                          ...
                          <tbody>
                            ...
                            if(!$result) {
                                ...
                                while($row = mysqli_fetch_assoc($result)) {
                                  ...
                                  title='Edit' onclick='editSubject({$row['sub_id']})'>
                                  <i class='fa fa-edit'></i>
                                  </button>
                                  <button type='button' class='btn btn-link btn-warning btn-md' data-bs-toggle='modal' title='Delete'>
                                  <a href='view_sub.php?delete_id={$row['sub_id']}' class='btn btn-link btn-danger btn-md' 
                                      onclick='return confirm(\"Are you sure you want to delete this Subject Details?\");'>
                              ...
                              <div class="modal-body">
                                <form id="editSubjectForm">
                                  ...
                                  <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="sub1" name="subject" placeholder="Enter Subject">
                                  </div>
                                </form>
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-primary" onclick="saveSubject()">Edit Details</button>
                              </div>
                            </div>
                          </div>
                        </div>
    ...
    function editSubject(subId) {
        $.ajax({
            url: 'view_sub.php',
            type: 'POST',
            data: { action: 'fetch', sub_id: subId },
            dataType: 'json',
            success: function(response) {
                if (response.status === "success") {
                    $("#sub_id").val(response.data.sub_id);
                    $("#class1").val(response.data.class);
                    $("#sub1").val(response.data.subject);
                    $("#editSubjectModal").modal('show');  // Open modal
                } else {
