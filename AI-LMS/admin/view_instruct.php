...
if ($action === 'fetch') {
    $stmt = $conn->prepare("SELECT * FROM instructor_tbl WHERE instructor_id = ?");
...
if ($action === 'update') {
    $stmt = $conn->prepare("UPDATE instructor_tbl SET instructor_name = ?, user_id = ?, class = ? WHERE instructor_id = ?");
    $stmt->bind_param("sisi", $_POST['instructor_name'], $_POST['user_id'], $_POST['class'], $_POST['instructor_id']);
...
<html lang="en">
  ...
  <body>
          ...
          <div class="page-inner">
            ...
            <?php
              ...
              if (isset($_GET['delete_id'])) {
                  ...
                  // Delete query
                  $query = "DELETE FROM `instructor_tbl` WHERE `instructor_id`='$instructor_id'";
                  $result = mysqli_query($conn, $query);

                  if ($result) {
                      echo "<script>alert('Instructor Details deleted successfully!'); window.location.href='view_instruct.php';</script>";
                  } else {
                      echo "<script>alert('Error deleting instructor details: " . mysqli_error($conn) . "');</script>";
                  }
              }
            ?>
                ...
                <div class="card">
                  <div class="card-header">
                    <div class="d-flex align-items-center">
                      <h4 class="card-title">View Instructor Details</h4>
                      <a href="add_instruct.php" class="btn btn-primary btn-round ms-auto">
                  ...
                  <div class="card-body">
                    ...
                    <table class="table table-hover table-striped table-bordered text-center" id="basic-datatables">
                      <thead>
                        <tr>                          
                          ...
                          <tbody>
                            ...
                            $query = "select * from `instructor_tbl`";
                            $result = mysqli_query($conn, $query);
                            ...
                            if(!$result) {
                                ...
                                while($row = mysqli_fetch_assoc($result)) {
                                  ...
                                  btn-md' data-bs-toggle='modal' data-bs-target='#editInstructorModal' 
                                  title='Edit' onclick='editInstructor({$row['instructor_id']})'>
                                  <i class='fa fa-edit'></i>
                                  </button>
                                  <a href='view_instruct.php?delete_id={$row['instructor_id']}' class='btn btn-link btn-danger btn-md' 
                                      onclick='return confirm(\"Are you sure you want to delete this Instructor Details?\");'>
                      ...
                      <div class="modal fade" id="editInstructorModal" tabindex="-1" aria-labelledby="editInstructorLabel" aria-hidden="true">
                        <div class="modal-dialog">
                          <div class="modal-content">
                            <div class="modal-header">
                              <h1 class="modal-title fs-5" id="editInstructorLabel">Edit Instructor Details</h1>
                            ...
                            <div class="modal-body">
                              <form id="editInstructorForm">
                                ...
                                <div class="mb-3">
                                  <label for="instructor_name1" class="form-label">Instructor Name</label>
                                  <input type="text" class="form-control" id="instructor_name1" name="instructor_name" placeholder="Enter Instructor Name">
                                </div>
                                <div class="mb-3">
                            ...
                            <div class="modal-footer">
                              ...
                              <button type="button" class="btn btn-primary" onclick="saveInstructor()">Save Changes</button>
                            </div>
                          </div>
                        </div>
                      </div>
    ...
    function editInstructor(instructorId) {
        $.ajax({
            url: 'view_instruct.php', // Same file for handling requests
            type: 'POST',
            data: { action: 'fetch', instructor_id: instructorId },
            dataType: 'json',
            success: function(response) {
                if (response.status === "success") {
                    $("#instructor_id").val(response.data.instructor_id);
                    $("#instructor_name1").val(response.data.instructor_name);
                    $("#user_id1").val(response.data.user_id);
                  
                    $("#class1").val(response.data.class);
                    $("#editInstructorModal").modal('show');  // Open the modal
                } else {
    ...
    function saveInstructor() {
        $.ajax({
            url: 'view_instruct.php',  // Same file for handling updates
            type: 'POST',
            data: {
                action: 'update',
                instructor_id: $("#instructor_id").val(),
                instructor_name: $("#instructor_name1").val(),
